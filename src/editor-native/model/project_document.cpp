...
bool ProjectDocument::removeSpriteAnimator(const SceneId& sceneId, EntityId id) {
    SceneInstanceDef* instance = mutableInstanceInScene(sceneId, id);
    if (!instance || !instance->spriteAnimator.has_value()) return false;
...
bool ProjectDocument::setSpriteAnimatorInitialClip(const SceneId& sceneId, EntityId id,
                                                   std::string clipId) {
    SceneInstanceDef* instance = mutableInstanceInScene(sceneId, id);
    if (!instance || !instance->spriteAnimator.has_value()) return false;
    instance->spriteAnimator->initialClipId = std::move(clipId);
    markDirty();
    return true;
}
...
bool ProjectDocument::setSpriteAnimatorAutoPlay(const SceneId& sceneId, EntityId id,
                                                bool autoPlay) {
    SceneInstanceDef* instance = mutableInstanceInScene(sceneId, id);
    if (!instance || !instance->spriteAnimator.has_value()) return false;
    instance->spriteAnimator->autoPlay = autoPlay;
    markDirty();
    return true;
}
