...
ProjectDoc makeProjectWithSetPosition() {
    ...
    numberRule.actions[0].block =
        Logic::makeDefaultBlock(Logic::kStateAdd, Logic::BlockKind::Action);
    for (LogicPropertyDef& property : numberRule.actions[0].block.properties)
        if (property.key == "key")
    ...
    booleanRule.actions[0].block =
        Logic::makeDefaultBlock(Logic::kStateToggle, Logic::BlockKind::Action);
    for (LogicPropertyDef& property : booleanRule.actions[0].block.properties)
        if (property.key == "key")
...
void testContextualProjectVariableCreation(
    ...
    const std::string numberAddress = "rule-state-number|action-1|a|0|key";
    ...
    CHECK(std::get<LogicVariableReference>(
        Logic::findProperty(afterCreate.rules[1].actions[0].block, "key")->value).id
        == "bonus");
    ...
    const LogicBoardDef& afterUndo =
        *coordinator.document().data().objectTypes.at("Hero").logicBoard;
    CHECK(std::get<LogicVariableReference>(
        Logic::findProperty(afterUndo.rules[1].actions[0].block, "key")->value).id
        == "score");
    CHECK(coordinator.redo().ok);
    frame(context, ui);

    const std::string booleanAddress = "rule-state-boolean|action-1|a|0|key";
...
void testEditingAnExistingExpression(Rml::Context& context, Rml::ElementDocument& document,
    ...
    const LogicPropertyAddress address{
        board.rules[0].id, board.rules[0].actions[0].id,
    ...
    target.actionId = board.rules[0].actions[0].id;
              ...
              std::get<LogicVec2Value>(
                  Logic::findProperty(
                      coordinator.document().data().objectTypes.at("Hero")
                          .logicBoard->rules[0].actions[0].block, "position")->value).x,
...
// ADR-0042: flat action controls remain encoding-independent and the retired
// Action Group controls never return to the markup.
void testFlatActionControlsUseValidGlyphs(
    Rml::Context& context, Rml::ElementDocument& document,
    EditorCoordinator& coordinator, EditorUi& ui) {
    CHECK(coordinator.apply(OpenLogicBoardIntent{"Hero"}).ok);
    frame(context, ui);

    Rml::Element* actionUp =
        findByAttribute(&document, "data-action", "move-logic-action-up");
    Rml::Element* actionDown =
        findByAttribute(&document, "data-action", "move-logic-action-down");

    const std::string upArrow = "\xE2\x86\x91";
    const std::string downArrow = "\xE2\x86\x93";
    CHECK(actionUp && actionUp->GetInnerRML() == upArrow);
    CHECK(actionDown && actionDown->GetInnerRML() == downArrow);
    CHECK(findByAttribute(
              &document, "data-action", "move-logic-action-branch-up") == nullptr);
    CHECK(document.GetInnerRML().find("Add Action Group") == std::string::npos);
    CHECK(document.GetInnerRML().find("Run") != std::string::npos);
}
...
int main() {
    ...
    testFlatActionControlsUseValidGlyphs(
