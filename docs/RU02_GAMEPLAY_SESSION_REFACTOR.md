...
**Stato (2026-07-21)**: [-] parzialmente completata — onestamente, non tutte le clausole. Fatto: `Application::tickFixedStep` (debito T-04, "eliminarlo quando tutti i call site sono migrati") è rimosso — l'unico call site (`app_loop.cpp::loopIteration`) chiama `mod_->gameplaySession->tickFixedStep(dt)` direttamente; `Application::dispatchGameplayCollisionTransitions` non esiste già da RU-02c (verificato via grep, nessun risultato). Aggiunto `gameplay-session-dependency-guard-test` (nuovo target CTest): scansiona `gameplay_session.h`/`.cpp` a runtime e fallisce se contengono `editor-api.h`, `RmlUi`, `Rml/`, `BeginDrawing` o `EndDrawing` — soddisfa la richiesta esplicita di "Test o regola CI (grep/include dependency guard)". Verificato manualmente (grep transitivo su tutti gli include, incluso il nuovo `scene_frame_snapshot.h`→`sprite_frame_resolve.h`/`editor-overlay-renderer.h`→`presentation_snapshot.h`) che nessun header raggiungibile da `gameplay_session.*` include raylib.h o RmlUi; il target CMake `game` non linka mai RmlUi (non esiste come dipendenza in tutto l'albero runtime-cpp).

**Non fatto, dichiarato debito con motivazione**: "ownership World/Logic/Script da `Application::Modules`" è soddisfatta nel senso di *ownership* (nessun `unique_ptr`, solo alias non-owning — pattern già accettato e verificato in RU-02e/f), ma **non** nel senso di "accessi pubblici ai moduli interni" — `GameplaySession` espone ancora `world()`, `physics()`, `entityGateway()`, `logicHost()`, `logicRuntime()`, `gameAPI()`, `luaHost()`, `sceneManager()`, `sceneMutation()` come accessor pubblici, e `Application::Modules` li usa per popolare i propri alias. Ridurre l'API pubblica a "sole porte esplicite" richiederebbe riscrivere ogni sito di chiamata in `app_project_lifecycle.cpp`/`app_scene_mutation.cpp`/`app_scene_invalidation.cpp`/`app_scene_render.cpp`/`app_bootstrap.cpp` per passare attraverso metodi dedicati invece che tramite gli alias cache-ati — una riscrittura di portata paragonabile a RU-02e/f nel suo complesso, non contenibile in questa passata senza rischio sproporzionato. Analogamente non toccati in questa passata: la duplicazione di `EngineContext` (D-11, già schedulata "RU-02h/post" nel debt register — non un impegno stretto di RU-02h) e il wiring gameplay dentro `EditorAPI` (rientra in RU-03, che sostituisce `EditorNative::PlaySession` con `GameplaySession` — l'editor non consuma ancora `GameplaySession` affatto). Questi restano debiti espliciti per una passata dedicata futura, non silenziosamente ignorati. Verificato: native 50/50 test verdi (incluso il nuovo dependency-guard test), WASM verde (`game.js` byte-identico — rimuovere un wrapper a una riga non cambia comportamento), editor build+test verde (2563+ asserzioni core).
