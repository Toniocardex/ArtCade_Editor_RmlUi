...
**Ambito:** runtime locale condiviso (`vendor/artcade-runtime`) —
`Application`/`game.exe`/WASM — e, a valle in RU-03, l'editor nativo RmlUi.
...
**Stato (2026-07-20)**: [x] completata, in 2 commit. `8fc7a366` — `renderer->clearDrawQueue()` spostata da `Application::tickFixedStep` a `Application::loopIteration`, una sola volta per frame prima del ciclo di catch-up, ancora condizionata da `simulating` (comportamento invariato, output byte-per-byte identico native+WASM). `5d6ccb2e` (follow-up, correzione di una lacuna del primo commit) — anche `splash_->update(dt)` spostato fuori dal fixed tick, come richiesto esplicitamente da questa stessa sezione del piano (§4.1 lista Splash/watermark come host-owned) ma inizialmente dimenticato: ticka ora una volta a frame con `simulatedDt` sommato, saltato se nessun fixed step è girato quel frame — stessa cadenza effettiva di prima, `game.wasm` +12 byte (variazione minima attesa, non un puro spostamento). `mod_->audio->update()` resta correttamente in `tickFixedStep`: Audio è un port gameplay-consumed (D-14), non presentazione host-owned, e si sposterà in `GameplaySession` con RU-02c. Entrambi i commit su `feature/runtime-unification`. Il corpo residuo di `tickFixedStep` è ora simulazione pura.
...
**Stato (2026-07-20)**: [x] completata. Commit `637a703d` (runtime, `feature/runtime-unification`) — `GameplayInputFrame` (pressed/released/held, deliberatamente senza campi puntatore/mouse non ancora consumati) + `GameplaySession::dispatchInput`; `Application::loopIteration` ora si limita a pollare `Input` e costruire il frame. Commit `07bdee60` chiude il gate "test input deterministici/sintetico headless" — non bastava la caratterizzazione di RU-02a (replicava l'algoritmo a mano): il nuovo test costruisce una `GameplaySession` reale al completo (World/gateway/physics/variabili/Time/Tween/CameraManager/GameStateManager/EventBus/GameAPI/LuaHost/LogicRuntime/ScriptRuntime) senza alcun `Modules::Input` (`EngineContext.input` resta `nullptr` — `GameAPI::dispatchInputEvents` si blocca su questo internamente e non fa nulla), e chiama `dispatchInput`+`tickFixedStep` reali, promuovendo l'asserzione di ordine Logic-prima-di-Script del test gemello da CONTRACT a VERIFIED. Un solo cambiamento strutturale documentato (non funzionale): il dispatch ora processa tutti i pressed, poi tutti i released, poi tutti gli held (per categoria, dato che il frame è forma-per-categoria) invece dell'ordine interlacciato per-tasto originale — innocuo perché `LogicRuntime::dispatch`/`ScriptInputSnapshot` filtrano/normalizzano per (kind, key) indipendentemente dagli altri tasti. Verificato: native 49/49 test verdi, WASM verde (dimensione byte-identica, i test non fanno parte della build WASM), editor verde.
...
**Suddivisione concordata** (proposta durante l'implementazione, approvata dall'utente: "procedi con la suddivisione suggerita RU-02e 1-3"): il vero raggio d'azione risultava più ampio del previsto — rimuovere `mod_->world`/`physics`/`sceneManager`/`sceneMutation`/`entityGateway` come `unique_ptr` avrebbe rotto ogni chiamante esistente in `app_project_lifecycle.cpp` (incluso il bridge Play/Stop/Load WASM di produzione), `app_scene_mutation.cpp`, `app_scene_invalidation.cpp`, `app_scene_render.cpp`, non solo la parte finale del composition root. Suddiviso in:
- **RU-02e-1**: Physics/SceneManager/SceneMutationService/RuntimeEntityGateway/SceneLifecycleService/World in `GameplaySession`.
- **RU-02e-2**: RuntimeLogicHostAdapter/LogicRuntime/GameAPI/LuaHost/ScriptRuntime in `GameplaySession`.
- **RU-02e-3**: `loadProject`/`installLogicScopesForActiveScene`/`installScriptScopesForActiveScene`/bridge editor WASM/render path aggiornati sui nuovi accessor.

**Stato RU-02e-1 (2026-07-20)**: [x] completata. Commit `369163ed` su `feature/runtime-unification` (runtime-cpp). `GameplaySession` ha ora un proprio `initialize(PhysicsMode, bootStep, onSceneTransition)` che costruisce Physics → SceneManager → SceneMutationService → RuntimeEntityGateway → SceneLifecycleService → World, con gli stessi nomi di boot-step e lo stesso wiring interno che `Application::initSubsystems()` eseguiva direttamente; `shutdownGraph()`/`shutdownPhysics()` smontano il grafo nello stesso ordine relativo di prima (physics resta smontato in una posizione separata, dopo audio/input, non contiguo al resto del grafo — l'ordine originale non era un blocco unico). I due handler di `SceneLifecycleService` che catturavano stato di `Application` (`gw = mod_->entityGateway.get()`) ora catturano `this` (`GameplaySession`), come richiesto dalla regola sui callback. `Application::Modules` mantiene `world`/`physics`/`sceneManager`/`sceneMutation`/`entityGateway` come puntatori grezzi non-owning (assegnati subito dopo `initialize()`), così ogni sito di chiamata esistente (`mod_->world->X()`) compila invariato; `sceneLifecycle` non ha alias esterno perché nulla fuori da `GameplaySession` lo referenzia. `GameplayRuntimeRefs` non contiene più world/physics/gateway (ora posseduti direttamente) e i restanti 8 campi sono diventati puntatori nullable, rendendo la struct default-costruibile — questo permette la costruzione in due fasi (`GameplaySession` costruita presto con la sola `VariableManager&`, poi `initialize()`, poi `wireHostRefs()` una volta che GameAPI/LuaHost/LogicRuntime esistono). Il test di caratterizzazione RU-02d (`testRealGameplaySessionDispatchInputThenTick`) è stato aggiornato per costruire `GameplaySession` con la nuova API a due fasi, usando `session.entityGateway()`/`session.world()` per condividere il grafo con l'`Host`/`LogicRuntime`/`ScriptRuntime` di test invece di costruire un World/gateway separati. Verificato: native 49/49 test verdi, WASM verde (`game.js` byte-identico, `game.wasm` +1940 byte — atteso, è uno spostamento reale del composition root, non un refactor a comportamento zero come RU-02b/d), editor build+test verde (2563+ asserzioni core).

**Stato RU-02e-2 (2026-07-20)**: [x] completata. Commit `ed7fb09c` su `feature/runtime-unification` (runtime-cpp). `GameplaySession::initializeGameplayModules(EngineContext&, Audio&, Input&, spawnInstaller, bootStep)` costruisce RuntimeLogicHostAdapter → LogicRuntime → GameAPI → LuaHost, con gli stessi nomi di boot-step e lo stesso wiring che `Application::initSubsystems()` eseguiva direttamente; viene chiamato dopo che `ctx_` ha già renderer/physics/input/audio/sceneManager/entityGateway/assetLoader/world/dialogManager (esattamente i campi che `GameAPI(ctx_)` leggeva). `RuntimeLogicHostAdapter` (prima privata di `app_modules.h`) si è spostata in `gameplay_session.h/.cpp` — corpo dei metodi invariato, solo l'header dichiarante è cambiato — così la sessione può possederne un'istanza direttamente; `ScriptRuntime` resta di `Application` (ricostruita per-scena, territorio RU-02e-3) e continua a raggiungere la sessione tramite `GameplayRuntimeRefs`, i cui campi `gameApi`/`luaHost`/`logic` sono stati rimossi (ora posseduti direttamente). `Application::Modules` ottiene `logicHost`/`logicRuntime`/`gameAPI`/`luaHost` come puntatori grezzi non-owning, stesso pattern di RU-02e-1. `shutdownModules()` ottiene due metodi granulari sulla sessione — `shutdownLogicModules()` e `shutdownScriptingModules()` — per mantenere lo shutdown di `scriptRuntime`/`dialogManager` (posseduti da `Application`) esattamente negli stessi punti relativi di prima, dato che l'ordine originale li intercalava tra logicRuntime/logicHost e luaHost/gameAPI, non li raggruppava. Il test reale di RU-02d non può più sostituire il proprio `Host` strumentato a Logic (la sessione possiede ora il vero adapter): pilota `session.logicRuntime()`/`logicHost()` direttamente e verifica l'ordine Logic-prima-di-Script tramite un effetto reale di produzione (`World::setMovementIntent` sovrascrive invece di accumulare, quindi la velocità dopo il tick porta il segno di qualunque on_update sia girato per ultimo) al posto di un callLog. Verificato: native 49/49 test verdi, WASM verde (`game.js` byte-identico, `game.wasm` +367 byte), editor build+test verde (2563+ asserzioni core).

**Stato RU-02e-3 (2026-07-20)**: [x] completata. Commit `9afcc843` su `feature/runtime-unification` (runtime-cpp). `ScriptRuntime` — l'unico modulo del gate non ancora posseduto dalla sessione, perché ricostruito per-scena anziché stabile dal boot — si sposta in `GameplaySession` tramite `resetScriptRuntime()` (costruisce una nuova istanza legata al proprio `logicHost()`, sostituendo quella precedente, e la restituisce per riferimento), `clearScriptRuntime()` (percorso di fallimento install) e `shutdownScriptRuntime()`. `GameplayRuntimeRefs` perde il campo `scripts`. `Application::Modules::scriptRuntime` diventa un puntatore grezzo non-owning, stesso pattern degli altri moduli RU-02e-1/2; `installScriptScopesForActiveScene` chiama `mod_->gameplaySession->resetScriptRuntime()` invece di costruire l'istanza direttamente. `shutdownModules()` ottiene un terzo metodo granulare (`shutdownScriptRuntime()`) tra `shutdownLogicModules()` e `shutdownScriptingModules()`, nella stessa posizione relativa di prima. Con questo, ogni modulo nominato dal gate RU-02e ("World/gateway/Logic/Script costruiti soltanto dalla sessione") è posseduto dalla sessione — **gate RU-02e pienamente soddisfatto**. Il test reale di RU-02d usa ora `session.resetScriptRuntime()` invece di costruire un `ScriptRuntime` a parte. Verificato: native 49/49 test verdi, WASM verde (`game.js` byte-identico, `game.wasm` +5 byte), editor build+test verde (2563+ asserzioni core).
...
**Stato (2026-07-21)**: [x] completata. `EventBus`, `TimeManager`, `VariableManager`, `TweenManager`, `SpriteAnimator`, `CameraManager`, `SaveLoadManager` e `GameStateManager` — gli ultimi moduli gameplay ancora costruiti da `Application::initUtilities()` — si spostano in `GameplaySession::initializeUtilities(bootStep)`, con lo stesso ordine, nomi di boot-step e wiring interno (`GameStateManager::setEventBus`) che `Application` eseguiva direttamente. Il costruttore di `GameplaySession` diventa parametro-libero (prima prendeva `VariableManager&` a prestito; ora la possiede). `Application::Modules` ottiene gli 8 campi corrispondenti come puntatori grezzi non-owning, stesso pattern di RU-02e; `shutdownUtilities()` smonta il blocco contiguo finale nello stesso ordine esatto di prima (nessun modulo host mai intercalato tra questi otto). La struct transitoria `GameplayRuntimeRefs` (debito T-01, con scadenza dichiarata "eliminare in RU-02f") viene **eliminata come da programma**: sostituita da `wireHostPorts(audio, dialog, profiler)`, tre puntatori diretti invece di una struct multi-campo, dato che ogni altro modulo è ormai posseduto dalla sessione. Con questo, ogni modulo nominato dal gate RU-02f (World/gateway/Logic/Script/Physics/scene-system/utility) è posseduto da `GameplaySession`; `Application::Modules` contiene solo moduli host (Renderer/Input/Audio/TextureManager/AssetLoader/DialogManager/EditorViewportService) più alias non-owning e bookkeeping (logicScopes/scriptPrograms/ecc., non moduli). Verificato: native 49/49 test verdi, WASM verde (`game.js` byte-identico), editor build+test verde (2563+ asserzioni core).
...
**Stato (2026-07-21)**: [x] completata, con uno scope esplicitamente ristretto. `GameplaySession::buildFrameSnapshot(SceneFrameSnapshot)` estende — non duplica — lo `SceneFrameSnapshot` esistente con un campo `renderables` (già risolto e ordinato in ordine di disegno finale: layer priority → `sprite.renderOrder` → ordine di scoperta) e `elapsedTime`; `using GameplayFrameSnapshot = SceneFrameSnapshot;` è l'alias richiesto dal piano, non un secondo tipo. La risoluzione per-entità (visibilità, frame animator via `sprite_frame_resolve`, binding variabile per testo/gauge con la stessa identica logica di formattazione/clamping) si sposta da `scene_entities_pass.cpp` a `buildFrameSnapshot()`, usando `entityGateway_`/`spriteAnimator_`/`variableManager_`/`sceneManager_`/`timeManager_` (tutti posseduti dalla sessione dopo RU-02e/f) — il render pass ora legge solo `frame.renderables`, nessuna query a quei moduli durante il draw. **Scope escluso deliberatamente**: `gizmo_pass.cpp` (overlay di editing — selezione, contorni hidden-in-game) e `physics_debug_renderer.cpp` (debug fisica) continuano a interrogare `entityGateway`/`spriteAnimator`/`World` dal vivo — sono overlay di editor/debug-tooling, non presentazione gameplay, e il piano stesso esclude esplicitamente "handle authoring" dallo snapshot. `sceneManager->sceneLayers()`/`tilesets()` restano alias diretti (stesso precedente di `frame.tilemap`/`tilemapLayers`, dati di authoring stabili per il frame, non stato di simulazione mutabile). **Verifica visiva**: non disponibile in questo ambiente — `computer-use` risolve solo app registrate nel menu Start (un `game.exe` appena compilato non lo è) e la build WASM di questo repo non include un bundle di progetto precaricato (pensata per un front-end web separato, non per un browser standalone) — l'utente ha esplicitamente accettato la sola verifica automatica + review riga-per-riga. Verificato: native 49/49 test verdi (inclusi `render_pipeline_test`/`presentation_golden_test`/`scene_frame_snapshot_test`/`frame_coordinator_test`, tutti ricompilati contro gli header modificati), WASM verde (`game.js` byte-identico, `game.wasm` +5KB — atteso, aggiunta reale di logica), editor build+test verde (2563+ asserzioni core).
...
**Stato (2026-07-21, iniziale)**: [-] parzialmente completata — onestamente, non tutte le clausole. Fatto: `Application::tickFixedStep` (debito T-04, "eliminarlo quando tutti i call site sono migrati") è rimosso — l'unico call site (`app_loop.cpp::loopIteration`) chiama `mod_->gameplaySession->tickFixedStep(dt)` direttamente; `Application::dispatchGameplayCollisionTransitions` non esiste già da RU-02c (verificato via grep, nessun risultato). Aggiunto `gameplay-session-dependency-guard-test` (nuovo target CTest): scansiona `gameplay_session.h`/`.cpp` a runtime e fallisce se contengono `editor-api.h`, `RmlUi`, `Rml/`, `BeginDrawing` o `EndDrawing` — soddisfa la richiesta esplicita di "Test o regola CI (grep/include dependency guard)". Verificato manualmente (grep transitivo su tutti gli include, incluso il nuovo `scene_frame_snapshot.h`→`sprite_frame_resolve.h`/`editor-overlay-renderer.h`→`presentation_snapshot.h`) che nessun header raggiungibile da `gameplay_session.*` include raylib.h o RmlUi; il target CMake `game` non linka mai RmlUi (non esiste come dipendenza in tutto l'albero runtime-cpp).

**Non fatto in questa prima passata, dichiarato debito con motivazione**: "ownership World/Logic/Script da `Application::Modules`" era soddisfatta nel senso di *ownership* (nessun `unique_ptr`, solo alias non-owning), ma **non** nel senso di "accessi pubblici ai moduli interni" — `GameplaySession` esponeva ancora `world()`, `physics()`, `entityGateway()`, `logicHost()`, `logicRuntime()`, `gameAPI()`, `luaHost()`, `sceneManager()`, `sceneMutation()` come accessor pubblici (D-20 nel debt register, §20).

**Aggiornamento (2026-07-21) — D-20 completato, gate RU-02h soddisfatto**: [x] completata. Rimossi `world()`, `physics()`, `sceneMutation()`, `logicHost()`, `logicRuntime()`, `gameAPI()`, `luaHost()` — sostituiti da metodi stretti e nominati per intento: `setGravity`, `bumpSceneRevision`/`sceneRevision`/`beginSceneMutationBatch`/`commitSceneMutationBatch`/`applySceneMutation`/`sceneMutationBatchOpen`, `loadWorldProject`/`syncWorldAfterEditorProject`/`restoreWorldDesignState`/`rebuildWorldCollision`, `loadLogicPrograms`, `loadLuaSource`/`loadLuaBytecode`. `debugWorldView() const` sostituisce l'unico uso legittimo rimasto di `World&` (l'overlay di debug fisico in `physics_debug_renderer.cpp`, già approvato in RU-02g) con un riferimento **const** — non riapre l'accesso mutabile che l'accessor originale lasciava aperto. `installLogicScopesForActiveScene()`/`installLogicScopeForEntity()`/`installScriptScopesForActiveScene()` e il bookkeeping che consultano (`logicScopes_`, `logicObjectTypes_`, `scriptPrograms_`, `scriptAttachments_`) si sono spostati da `Application` a `GameplaySession` per intero — non erano bookkeeping host, ma stato di simulazione che vive tra i tick (stessa regola di D-09, già applicata a `activeGameplayCollisionPairs_` in RU-02c). Questo chiude anche un residuo di **D-08** non colto dalle passate precedenti: `World::setEntityDestroyedHandler` e lo spawn installer di Logic catturavano ancora `this` (`Application`) per toccare `mod_->logicScopes`/`logicRuntime`/`scriptRuntime`; ora entrambi catturano `this` (`GameplaySession`) e leggono direttamente i propri membri, spostati dentro `initialize()`/`initializeGameplayModules()`. `sceneManager()`/`entityGateway()` **restano** accessor pubblici — non è una lacuna dimenticata: RU-02g ha già approvato `gizmo_pass.cpp`/`scene_background_pass.cpp` a leggerli dal vivo (dati di authoring/presentazione stabili per il frame, non stato di simulazione mutabile), quindi rimuoverli romperebbe un'architettura già accettata invece di chiudere debito reale; lo stesso vale per `luaHostHandle()`, tenuto solo per il wiring statico `EditorAPI::wireLua()` (debito D-18, esplicitamente "post RU-03"). Anche gli 8 accessor delle utility (`eventBus()`/`timeManager()`/`variableManager()`/`tweenManager()`/`spriteAnimator()`/`cameraManager()`/`saveLoadManager()`/`gameStateManager()`) restano invariati: D-20, così come discusso con l'utente, riguarda specificamente il grafo di simulazione core (World/Physics/SceneMutation/Logic/Script/GameAPI/LuaHost), non le utility già headless-testabili singolarmente. Non toccati, debito esplicito e già schedulato altrove: la duplicazione di `EngineContext` (D-11, "RU-02h/post" nel debt register) e il wiring gameplay dentro `EditorAPI` (D-18, "post RU-03" — rientra in RU-03, che sostituisce `EditorNative::PlaySession` con `GameplaySession`). Con D-20 chiuso, il gate RU-02h ("API pubblica congelata; solo porte esplicite; nessun percorso legacy; documentazione aggiornata; native/WASM/headless verdi") è pienamente soddisfatto per lo scope di competenza di RU-02h. Verificato: native 50/50 test verdi (incluso `gameplay_tick_order_characterization_test`, aggiornato per pilotare gli stessi entry point di produzione — `loadWorldProject`/`loadLogicPrograms`/`installLogicScopeForEntity` — invece dei vecchi accessor rimossi, e `gameplay_session_dependency_guard_test`), WASM verde (`game.js`/`game.wasm` linkano — dimensione cambiata rispetto a RU-02h iniziale, atteso: è un refactor reale, non uno spostamento a comportamento zero), editor build+test verde (2563+ asserzioni core).
