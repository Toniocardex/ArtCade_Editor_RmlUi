...
Implemented in the native RmlUi editor with project format **14**.

- Persistent authority: `ProjectDocument` / `SceneInstanceDef::boxCollider2DOverride`.
- Runtime authority: `resolveEffectiveBoxCollider2D`; `CollisionBody` remains derived session scratch.
- Intent/Command: `SetInstanceOriginFromColliderAnchorCommand` performs one atomic position + sparse-offset mutation.
- Invariants: instance override contains `offset` only; override without Object Type capability is invalid; empty overrides are rejected/erased; world-space collider bounds are preserved by the shared pure helper.
- Undo/dirty: one command, one revision, exact restoration of position and prior sparse override; no-op creates neither revision nor history.
- Edit/Play: all authoring commands are rejected by the coordinator while Play is active; runtime materialisation consumes the effective collider.
- Layer lock: checked on first apply only; Undo/Redo remain deterministic.
- Geometry priority: Sprite > populated Tilemap > enabled effective BoxCollider2D > world Text/Gauge > placeholder, shared by picking/outline/gizmo policy.
- UX: the Inspector exposes a design-system 3×3 `Entity Origin` anchor grid, provenance badge, and reset. Object Type collider fields remain explicitly separate.
- Deliberate boundary: direct draggable origin manipulation and Object-Type-wide Edit Origin remain follow-ups; the required instance preset path is complete.
